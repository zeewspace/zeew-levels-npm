...
describe("MemoryAdapter", () => {
  ...
  describe("findUser", () => {
    ...
    it("returns user after upsert", async () => {
      await adapter.upsertUser({ user: "u1", guild: "g1" }, { xp: 10, level: 2, totalXp: 100 });
      const result = await adapter.findUser({ user: "u1", guild: "g1" });
      expect(result).toEqual(expect.objectContaining({ user: "u1", guild: "g1", xp: 10, level: 2, totalXp: 100 }));
    });

    it("returns a copy, not a reference", async () => {
      await adapter.upsertUser({ user: "u1", guild: "g1" }, { xp: 10, level: 2 });
  ...
  describe("upsertUser", () => {
    it("creates a new user", async () => {
      await adapter.upsertUser({ user: "u1", guild: "g1" }, { xp: 5, level: 1 });
    ...
    it("updates existing user", async () => {
      await adapter.upsertUser({ user: "u1", guild: "g1" }, { xp: 5, level: 1 });
      await adapter.upsertUser({ user: "u1", guild: "g1" }, { xp: 15, level: 3 });
      const user = await adapter.findUser({ user: "u1", guild: "g1" });
  ...
  describe("deleteUser", () => {
    ...
    it("does not affect other users", async () => {
      await adapter.upsertUser({ user: "u1", guild: "g1" }, { xp: 5, level: 1 });
      await adapter.upsertUser({ user: "u2", guild: "g1" }, { xp: 10, level: 2 });
      await adapter.deleteUser({ user: "u1", guild: "g1" });
  ...
  describe("getLeaderboard", () => {
    ...
    it("filters by guild", async () => {
      await adapter.upsertUser({ user: "a", guild: "g1" }, { xp: 10, level: 5 });
      await adapter.upsertUser({ user: "b", guild: "g2" }, { xp: 10, level: 10 });
      const lb = await adapter.getLeaderboard("g1", 10);
    ...
    it("respects limit", async () => {
      for (let i = 0; i < 20; i++) {
        await adapter.upsertUser({ user: `u${i}`, guild: "g1" }, { xp: i, level: i });
      }
  ...
  describe("reset", () => {
    it("clears all data", async () => {
      await adapter.upsertUser({ user: "a", guild: "g1" }, { xp: 10, level: 1 });
      await adapter.upsertUser({ user: "b", guild: "g2" }, { xp: 20, level: 2 });
      adapter.reset();
